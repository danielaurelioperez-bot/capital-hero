...
const Finances: React.FC = () => {
  ...
  const [dismissedRecurringWarning, setDismissedRecurringWarning] = useState(false); // New state for warning dismissal
  ...
  const handleSave = (data: Omit<Transaction, 'id'>) => { // Updated data type
    if (editingTx) {
  ...
  const renderEmptyState = (type: 'income' | 'expense') => (
    <div className="text-center py-12 flex flex-col items-center justify-center text-slate-300 border-2 border-dashed border-slate-100 rounded-3xl bg-white space-y-4">
        ...
        <button 
            ...
            className={`w-4/5 py-3 rounded-xl text-sm font-bold text-white shadow-lg active:scale-95 transition-transform mt-4 
  ...
  return (
    <div className="pb-8 space-y-6 pt-2">
      ...
      <div className="flex items-center gap-4 px-2">
        <Link to="/settings" className="p-2 bg-slate-100 rounded-xl text-slate-500 hover:bg-slate-200 border-b-4 border-transparent active:border-b-0 active:translate-y-0.5 transition-all"
      ...
      <div className="bg-slate-900 rounded-3xl p-6 text-white shadow-xl mx-1 relative overflow-hidden border-b-8 border-slate-800">
        ...
        <div className="flex justify-between items-end mb-6 relative z-10">
            ...
            <div className={`px-3 py-1 rounded-lg text-xs font-black uppercase tracking-wide ${summary.net >= 0 ? 'bg-green-500 text-slate-900' : 'bg-rose-500 text-white'}`}>
      ...
      <div className="bg-white rounded-3xl border-2 border-slate-200 p-6 mx-1 shadow-sm">
      ...
      {!hasRecurringExpenses && !dismissedRecurringWarning && (
        <div className="bg-amber-50 border-2 border-amber-200 p-6 rounded-3xl mx-1 shadow-sm animate-in fade-in slide-in-from-top-2 duration-300">
            <div className="flex items-start gap-4 mb-4">
                ...
                <div>
                    ...
                    <p className="text-sm font-medium text-amber-700 leading-relaxed">
                        Your "Safe to Spend" is calculated based on known recurring bills. Please ensure all your regular payments (rent, subscriptions, loans) are logged for accurate advice.
                    </p>
                </div>
            </div>
            <div className="flex flex-col gap-3">
                <button 
                    onClick={() => {
                        openSheet('regular'); // Open MoneySheet to the 'regular' view
                    }}
      ...
      <div className="space-y-4">
        {/* Toggle Tabs */}
        <div className="flex p-1 bg-slate-100 rounded-2xl mx-1">
            <button 
                onClick={() => setActiveTab('income')}
                className={`flex-1 py-3 rounded-xl text-sm font-black uppercase tracking-wider transition-all ${activeTab === 'income' ? 'bg-white text-[#22be54] shadow-sm' : 'text-slate-400 hover:text-slate-500'}`}
            ...
            <button 
                ...
                className={`flex-1 py-3 rounded-xl text-sm font-black uppercase tracking-wider transition-all ${activeTab === 'expense' ? 'bg-white text-rose-500 shadow-sm' : 'text-slate-400 hover:text-slate-500'}`}
        ...
        <div className="space-y-3 min-h-[200px] px-1">
            {transactions.length === 0 ? (
                renderEmptyState(activeTab)
            ) : (
                transactions.map((t) => (
                    <div key={t.id} className="bg-white border-b-4 border-2 border-slate-200 rounded-2xl p-4 flex justify-between items-center group active:border-b-2 active:translate-y-0.5 transition-all">
                        <div className="flex items-center gap-4">
                            <div className={`p-3 rounded-xl ${activeTab === 'income' ? 'bg-green-100 text-green-600' : 'bg-rose-100 text-rose-600'}`}>
                                {activeTab === 'income' ? <Shield size={20} /> : <Zap size={20} />}
                            </div>
                            <div>
                                <div className="font-black text-slate-700 text-sm">
                                    {t.category === 'Toxic' ? 'Impulse Buy' : t.category === 'Non-essentials' ? 'Lifestyle' : t.category}
                                </div>
